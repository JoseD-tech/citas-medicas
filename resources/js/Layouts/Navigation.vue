    ...
    <div :class="$page.props.showingMobileMenu ? 'translate-x-0 ease-out' : '-translate-x-full ease-in'"
        ...
        <div class="flex justify-center items-center mt-8">
            <div class="flex items-center">
                ...
                <span class="mx-2 text-2xl font-semibold text-white">Citas</span>
            </div>
        </div>
        ...
        <nav class="mt-10" x-data="{ isMultiLevelMenuOpen: false }">
            ...
            <nav-link :href="route('citas.index')" :active="route().current('citas.index')">
                <template #icon>
            ...
            <nav-link :href="route('paciente.index')" :active="route().current('paciente.index')">
                <template #icon>
                    <svg class="w-5 h-5" aria-hidden="true" fill="none" stroke-linecap="round" stroke-linejoin="round"
                         stroke-width="2" viewBox="0 0 24 24" stroke="currentColor">
                        <path
            ...
            <nav-link v-if="$page.props.auth.user.roles[0].name === 'admin' || $page.props.auth.user.roles[0].name === 'doctor'" :href="route('doctor.index')" :active="route().current('doctor.index')">
                <template #icon>
                    <svg class="w-5 h-5" aria-hidden="true" fill="none" stroke-linecap="round" stroke-linejoin="round"
                         stroke-width="2" viewBox="0 0 24 24" stroke="currentColor">
                        <path
            ...
            <nav-link v-if="$page.props.auth.user.roles[0].name === 'admin'" :href="route('users.index')" :active="route().current('users.index')">
                <template #icon>
